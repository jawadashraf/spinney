 ...
 * @property EnquiryDirection $direction
 * @property EnquiryCallType $call_type
 * @property CallerType $caller_type
 * @property EnquiryCategory $category
 * @property EnquirySourceType $source
 * @property EnquiryOutcome|null $outcome
...
final class Enquiry extends Model
{
    ...
    {
        return $this->getAttribute('status') === EnquiryStatus::OPEN && $this->people_id !== null;
    }

    public function isOutbound(): bool
    {
        return $this->getAttribute('direction') === EnquiryDirection::OUTBOUND;
    }

    public function isEmergency(): bool
    {
        return $this->getAttribute('call_type') === EnquiryCallType::EMERGENCY;
    }
    ...
    protected static function booted(): void
    {
        self::saving(function (Enquiry $enquiry): void {
            if ($enquiry->getAttribute('call_type') === EnquiryCallType::EMERGENCY) {
                $enquiry->safeguarding_flags = true;
            }

            if ($enquiry->people_id === null) {
                $enquiry->caller_type = CallerType::ANONYMOUS->value;
            } else {
                $person = People::withTrashed()->find($enquiry->people_id);
                if ($person?->is_service_user) {
                    $enquiry->caller_type = CallerType::SERVICE_USER->value;
                } else {
                    $enquiry->caller_type = CallerType::KNOWN_PERSON->value;
                }
            }
        });
    }
